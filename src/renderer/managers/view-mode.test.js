...
describe('ViewModeManager', () => {
    ...
    describe('view mode application', () => {
        ...
        it('should emit view-mode:changed event on event bus when mode changes', async () => {
            const eventCallback = vi.fn();
            const cleanup = eventBus.on('view-mode:changed', eventCallback);

            await viewModeManager.setViewMode('editor');

            expect(eventCallback).toHaveBeenCalledWith({ mode: 'editor' });
            expect(eventCallback).toHaveBeenCalledTimes(1);

            cleanup();
        });

        it('should emit view-mode:changed with correct mode for each change', async () => {
            const eventCallback = vi.fn();
            const cleanup = eventBus.on('view-mode:changed', eventCallback);

            await viewModeManager.setViewMode('editor');
            expect(eventCallback).toHaveBeenLastCalledWith({ mode: 'editor' });

            await viewModeManager.setViewMode('preview');
            expect(eventCallback).toHaveBeenLastCalledWith({ mode: 'preview' });

            await viewModeManager.setViewMode('split');
            expect(eventCallback).toHaveBeenLastCalledWith({ mode: 'split' });
        ...
        it('should not emit view-mode:changed for invalid mode', async () => {
            const eventCallback = vi.fn();
            const cleanup = eventBus.on('view-mode:changed', eventCallback);
